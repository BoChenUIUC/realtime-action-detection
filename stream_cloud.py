...
parser.add_argument('--onlyStreaming', default=False, type=str2bool, help='Whether do action loc or not')
...
time_offset = 13.489#15.73,16.19
...
def drawTubes(xmldata,frames,gt_label):
    dt_tubes = sort_detection(xmldata)
    num_detection = len(dt_tubes['class'])
    dt_labels = dt_tubes['class']
    ...
    result_str = struct.pack(">L", num_detection)
    for dtind in range(num_detection):
        # frame number range
        dt_fnr = dt_tubes['framenr'][dtind]['fnr']
        # bounding boxes
        ...
        for fn,bb in zip(dt_fnr,dt_bb):
            ...
            result_str += struct.pack(">L", size) + struct.pack(">L", dt_label) + struct.pack(">L", gt_label) + data
            break
    time_str = str(datetime.datetime.now()+datetime.timedelta(seconds=time_offset))
    result_str = str.encode(time_str) + result_str
    args.action_socket.sendall(result_str)


def process_video_result(video_result,annot_map):
    ...
    t3 = time.perf_counter()
    drawTubes(xmldata,frames,gt_label)
...
def test_net(net, save_root, exp_name, input_type, dataset, iteration, num_classes, outfile, thresh=0.5 ):
    ...
    # val_data_loader = data.DataLoader(dataset, 4, num_workers=args.num_workers,
    ...
    # # connect to server
    # args.action_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # args.action_socket.connect(('127.0.0.1', 8488))
    # # args.action_socket.connect(('192.168.251.195', 8488))
    # connection = args.action_socket.makefile('wb')
    # print('Connected.')
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            images, targets, img_indexs = [],[],[]
            for j in range(batch_size):
                idx = (itr+j)%num_images
                image,target,index = dataset[idx]
                images.append(image)
                targets.append(target)
                img_indexs.append(index)
            images = torch.stack(images, 0)
            itr += batch_size
            if itr>=num_images:itr-=num_images
            ...
            for b in range(batch_size):
                ...
                if (video_id != pre_video_id) and (len(video_result['data']) > 0):
                    ...
                    process_video_result(video_result,annot_map)
                    frame_level_time = 0
                    frame_level_cnt = 0
                    annot_map = {}
                    video_result['data'] = []
...
def display_frames(disp_queue):
    if args.showImage:
        while True:
            while disp_queue:
                frame = disp_queue.popleft()
                b,g,r = cv2.split(frame)
                frame_rgb = cv2.merge((r,g,b))
                cv2.imshow('Cloud receiving window',frame_rgb)
                cv2.waitKey(33)
            time.sleep(1)

def stream_receiver(frame_queue,disp_queue):
    tmpfile = "images/tmp.avi"
    ...
    # connect to edge
    args.action_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    args.action_socket.connect((args.edgeAddr, args.actionPort))
    # args.action_socket.connect(('192.168.251.195', 8488))
    ...
    while True:
        # open file
        myfile = open(tmpfile, 'wb')
        # write data to file
        # receive timestamp
        while len(data) < 26:
            data += conn.recv(4096)
        # start receiving
        cloud_recv_time_start = datetime.datetime.now() + datetime.timedelta(seconds=time_offset)
        edge_send_time = datetime.datetime.strptime(data[:26].decode(), "%Y-%m-%d %H:%M:%S.%f")
        data = data[26:]
        # read seg size
        while len(data) < payload_size:
            data += conn.recv(BUFSIZE)
        seg_size = struct.unpack(">L", data[:payload_size])[0]
        data = data[payload_size:]
        # read seg data
        while len(data) < seg_size:
            tmp_str = conn.recv(BUFSIZE)
            if not tmp_str:break
            data += tmp_str
        myfile.write(data)
        myfile.close()
        # finish receiving
        cloud_recv_time_end = datetime.datetime.now() + datetime.timedelta(seconds=time_offset)
        data = data[seg_size:]
        ...
        frame_queue += received_frames
        # display frames
        if args.showImage:
            disp_queue += received_frames
            ...
            #     cv2.waitKey(33)
        # calculate metrics
        total_seg_size += seg_size+26+payload_size
        total_seg_time += (cloud_recv_time_end - edge_send_time).total_seconds()
        total_latency += (cloud_recv_time_start - edge_send_time).total_seconds()
        total_dec_time += dec_end - dec_start
        seg_cnt += 1
        bw = total_seg_size/total_seg_time/1000000
        latency = total_latency/seg_cnt
        dec_time = total_dec_time/seg_cnt
        print("[STATUS] Stream throughput:{:1.3f}MB/s, network latency:{:1.6f}s, decoding:{:1.3f}s".format(bw,latency,dec_time))
    conn.close()

def init_streaming(frame_queue,disp_queue):    
    disp_thread = threading.Thread(target=display_frames, args=(disp_queue,))
    disp_thread.start()
    x = threading.Thread(target=stream_receiver, args=(frame_queue,disp_queue,))
    x.start()
    print("Streaming initialized.")
    if args.onlyStreaming:
        x.join()

def process_frame_queue(frame_queue,dataset,net):
    #  00011
    image_ids = dataset.ids
    batch_size = 4
    num_images = len(dataset)
    ...
    with torch.no_grad():
        ...
        while itr<num_images:
            while batch_size > len(frame_queue):
                pass
            torch.cuda.synchronize()
            ...
            images, targets, img_indexs = [],[],[]
            for j in range(batch_size):
                image = torch.cuda.FloatTensor(frame_queue[j])
                for ch in range(0,3):
                    image[:,:,ch] -= args.means[2-ch]
                image = image.permute(2, 0, 1)
                _,target,index = dataset[itr+j]
                images.append(image)
                targets.append(target)
                img_indexs.append(index)
            images = torch.stack(images, 0)
            ...
            for b in range(batch_size):
                ...
                if (video_id != pre_video_id) and (len(video_result['data']) > 0):
                    ...
                    video_result['video_id'] = pre_video_id
                    fps = frame_level_cnt/frame_level_time
                    tube_link_time = process_video_result(video_result,annot_map)
                    print('[RESULT]',pre_video_id,pre_video_name,"total frames:{:03d}, total time:{:0.3f}s, frame-level:{:.1f}fps, tube linking:{:.3f}s".format(len(video_result['data']),frame_level_time+tube_link_time,fps,tube_link_time))
                    frame_level_time = 0
                    frame_level_cnt = 0
                    annot_map = {}
                    video_result['data'] = []
    ...
    for iteration in [int(itr) for itr in args.eval_iter.split(',') if len(itr)>0]:
        ...
        if args.dataset == 'ucf24':
            dataset = OmniUCF24(args.data_root, 'test', BaseTransform(300, args.means), AnnotationTransform(), 
                                    input_type=args.input_type, outshape=args.outshape, full_test=True, use_npy=True)
        else:
        ...
        frame_queue = deque()
        disp_queue = deque()
        init_streaming(frame_queue, disp_queue)
        
        # process
        process_frame_queue(frame_queue,dataset,net)
